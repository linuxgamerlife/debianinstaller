...
class Config:
    ...
    root_size: str = (
        ""  # only used when separate_home=True; prompted with disk size hint
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="base package profile",
    )
    parser.add_argument(
        "--confirm-disk",
        help="must exactly match --disk to confirm destructive install",
    )
    parser.add_argument("--log-file", help="optional command log")
    parser.add_argument(
        "--interactive", action="store_true", help="launch interactive setup"
    )
    return parser
    ...
    while True:
        ...
        if choice.isdigit():
            ...
            if n == 1:
                print("\nAvailable disks:\n")
                subprocess.run(
                    ["lsblk", "-o", "NAME,SIZE,TYPE,MOUNTPOINT"], check=False
                )
                config.disk = prompt_disk(config.disk)
            elif n == 2:
                config.hostname = prompt_text("Hostname", config.hostname)
...
def prompt_disk(current: str) -> str:
    while True:
        ...
        confirm = (
            input(f"Use {value}? This will erase all data on it. [y/N]: ")
            .strip()
            .lower()
        )
        if confirm == "y":
...
def render_summary_menu(config: Config) -> str:
    swap_display = (
        config.swap_type
        if config.swap_type == "none"
        else f"{config.swap_type} ({config.swap_size})"
    )
    home_display = (
        f"yes (root: {config.root_size}, home: rest)" if config.separate_home else "no"
    )
    return "\n".join(
        [
...
def prompt_filesystem(current: str) -> str:
    ...
    return {"1": "ext4", "2": "btrfs", "3": "xfs"}.get(
        choice,
        current
        if choice == ""
        else (choice if choice in ("ext4", "btrfs", "xfs") else current),
    )
...
def prompt_home(config: Config) -> Config:
    answer = (
        input(
            f"Separate /home partition? [current: {'yes' if config.separate_home else 'no'}] [y/N]: "
        )
        .strip()
        .lower()
    )
...
def prompt_audio(current: str) -> str:
    ...
    return {"1": "pipewire", "2": "pulseaudio", "3": "none"}.get(
        choice,
        current
        if choice == ""
        else (choice if choice in ("pipewire", "pulseaudio", "none") else current),
    )
...
def prompt_network(current: str) -> str:
    ...
    return {"1": "networkmanager", "2": "systemd-networkd"}.get(
        choice,
        current
        if choice == ""
        else (choice if choice in ("networkmanager", "systemd-networkd") else current),
    )
...
def prompt_desktop(current: str) -> str:
    ...
    return {"1": "none", "2": "niri-noctalia"}.get(
        ...
        current
        if choice == ""
        else (choice if choice in ("none", "niri-noctalia") else current),
    )
    ...
    if config.execute:
        ...
        if not looks_like_vm() and not config.skip_vm_check:
            raise InstallerError(
                "apply mode is blocked because this host does not look like a VM"
            )
        if config.confirm_disk != config.disk:
            raise InstallerError(
                "apply mode requires --confirm-disk to exactly match --disk"
            )
        ...
        if mountpoint_busy(Path(config.target_mount)):
            raise InstallerError(
                f"target mount path appears busy: {config.target_mount}"
            )
...
def collect_warnings(config: Config) -> list[str]:
    ...
    if not looks_like_vm():
        warnings.append(
            "host does not look like a VM; this installer is intended only for disposable VMs"
        )
    if not Path("/sys/firmware/efi").exists():
    ...
    if mountpoint_busy(Path(config.target_mount)):
        warnings.append(
            f"target mount path already exists and may be in use: {config.target_mount}"
        )
    if os.geteuid() != 0:
    ...
    if config.execute:
        warnings.append(
            "apply mode will install live-environment prerequisites: "
            + ", ".join(LIVE_PREREQUISITE_PACKAGES)
        )
    return warnings
...
def partition_disk(state: State) -> None:
    config = state.config
    efi_size = (
        config.efi_size if config.efi_size.startswith("+") else f"+{config.efi_size}"
    )
    lines: list[str] = ["g"]
    ...
    if config.swap_type == "partition":
        swap_size = (
            config.swap_size
            if config.swap_size.startswith("+")
            else f"+{config.swap_size}"
        )
        lines += ["n", "", "", swap_size]
    ...
    if config.separate_home:
        root_size = (
            config.root_size
            if config.root_size.startswith("+")
            else f"+{config.root_size}"
        )
        lines += ["n", "", "", root_size]
    else:
...
def format_filesystems(state: State) -> None:
    ...
    if fs == "ext4":
        run_command(
            ["/sbin/mkfs.ext4", "-F", config.root_partition],
            phase="format-root",
            state=state,
        )
    elif fs == "btrfs":
        run_command(
            ["mkfs.btrfs", "-f", config.root_partition],
            phase="format-root",
            state=state,
        )
    elif fs == "xfs":
        run_command(
            ["mkfs.xfs", "-f", config.root_partition], phase="format-root", state=state
        )

    run_command(
        ["/sbin/mkfs.fat", "-F32", config.efi_partition],
        phase="format-efi",
        state=state,
    )
    ...
    if config.home_partition:
        run_command(
            ["/sbin/mkfs.ext4", "-F", config.home_partition],
            phase="format-home",
            state=state,
        )
    ...
    if config.swap_partition:
        run_command(
            ["/sbin/mkswap", config.swap_partition], phase="format-swap", state=state
        )
...
def mount_target(state: State) -> None:
    ...
    if config.filesystem == "btrfs":
        ...
        run_command(
            ["mount", config.root_partition, config.target_mount],
            phase="mount-root-flat",
            state=state,
        )
        run_command(
            ["btrfs", "subvolume", "create", f"{config.target_mount}/@"],
            phase="btrfs-subvol-root",
            state=state,
        )
        if not config.separate_home:
            run_command(
                ["btrfs", "subvolume", "create", f"{config.target_mount}/@home"],
                phase="btrfs-subvol-home",
                state=state,
            )
        run_command(
            ["umount", config.target_mount], phase="mount-root-flat-umount", state=state
        )
        run_command(
            [
                "mount",
        ...
        if not config.separate_home:
            run_command(
                ["mkdir", "-p", f"{config.target_mount}/home"],
                phase="mkdir-home",
                state=state,
            )
            run_command(
    ...
    if config.home_partition:
        ...
        run_command(
            ["mount", config.home_partition, f"{config.target_mount}/home"],
            phase="mount-home",
            state=state,
        )
    ...
    run_command(
        ["mount", "-t", "vfat", config.efi_partition, config.efi_mount],
        phase="mount-efi",
        state=state,
    )
...
def bootstrap(state: State) -> None:
    config = state.config
    run_command(
        ["debootstrap", config.release, config.target_mount, config.mirror],
        phase="bootstrap",
        state=state,
    )
...
def mount_virtual(state: State) -> None:
    ...
    run_command(
        ["mount", "-t", "proc", "proc", f"{target}/proc"],
        phase="mount-virtual",
        state=state,
    )
    run_command(
        ["mount", "-t", "sysfs", "sysfs", f"{target}/sys"],
        phase="mount-virtual",
        state=state,
    )
    run_command(
        ["mount", "--rbind", "/dev", f"{target}/dev"],
        phase="mount-virtual",
        state=state,
    )
...
def install_live_prerequisites(state: State) -> None:
    ...
    run_command(
        ["apt-get", "install", "-y", *LIVE_PREREQUISITE_PACKAGES, *extra],
        phase="host-install-prereqs",
        state=state,
    )
...
def write_sources(state: State) -> None:
    ...
    run_command(
        [
            "bash",
            "-c",
            f"cat > {target}/etc/apt/sources.list.d/debian.sources <<'EOF'\n{main_sources}\nEOF",
        ],
        phase="sources-main",
        state=state,
    )
    run_command(
        [
            "bash",
            "-c",
            f"cat > {target}/etc/apt/sources.list.d/debian-security.sources <<'EOF'\n{security_sources}\nEOF",
        ],
        phase="sources-security",
        state=state,
    )
    # Remove the legacy sources.list left by debootstrap
    run_command(
        ["rm", "-f", f"{target}/etc/apt/sources.list"],
        phase="sources-cleanup",
        state=state,
    )
    ...
    if config.desktop == "niri-noctalia":
        ...
        run_command(
            [
                "bash",
                "-c",
                f"mkdir -p {keyring_dir} && curl -fsSL https://pkg.noctalia.dev/gpg.key | gpg --dearmor -o {keyring_dir}/noctalia.gpg",
            ],
        ...
        run_command(
            [
                "bash",
                "-c",
                f"cat > {target}/etc/apt/sources.list.d/noctalia.sources <<'EOF'\n{noctalia_sources}\nEOF",
            ],
            phase="sources-noctalia",
            state=state,
        )
...
def setup_graphical_target(state: State) -> None:
    ...
    if config.desktop == "niri-noctalia":
        ...
        if config.execute and not niri_session.exists():
            print(
                "[setup-graphical] niri-session not found — source build was skipped, setting multi-user.target"
            )
            run_in_chroot(
                state,
                ["systemctl", "set-default", "multi-user.target"],
                phase="default-target",
            )
            return
...
def install_packages(state: State) -> None:
    ...
    run_in_chroot(
        ...
        phase="install-firmware",
    )

    packages = [
        "linux-image-amd64",
        "systemd-sysv",
        *PACKAGE_PROFILES[config.package_profile],
    ]
...
def build_from_source(state: State) -> None:
    ...
    if state.config.execute:
        ...
        if answer != "y":
            ...
            print(
                "See the README for instructions on installing a desktop environment manually."
            )
            run_in_chroot(
                state,
                ["systemctl", "set-default", "multi-user.target"],
    ...
    run_in_chroot(
        state, ["apt", "install", "-y", *build_deps], phase="install-build-deps"
    )
...
def configure_system(state: State) -> None:
    ...
    run_in_chroot(
        ...
        [
            "bash",
            "-lc",
            "printf 'en_US.UTF-8 UTF-8\\n' > /etc/locale.gen && locale-gen && update-locale LANG=en_US.UTF-8",
        ],
        phase="locale",
    )
    ...
    if config.network_backend == "systemd-networkd":
        run_in_chroot(
            state, ["systemctl", "enable", "systemd-networkd"], phase="services"
        )
        run_in_chroot(
            state, ["systemctl", "enable", "systemd-resolved"], phase="services"
        )
        ...
        run_in_chroot(
            ...
            [
                "bash",
                "-lc",
                f"mkdir -p /etc/systemd/network && cat > /etc/systemd/network/20-wired.network <<'EOF'\n{networkd_conf}\nEOF",
            ],
            phase="networkd-config",
        )
    elif config.package_profile == "standard-tty":
        run_in_chroot(
            state, ["systemctl", "enable", "NetworkManager"], phase="services"
        )
    ...
    if config.swap_type == "swapfile":
        run_in_chroot(
            ...
            [
                "bash",
                "-lc",
                f"touch /swapfile && chattr +C /swapfile && fallocate -l {config.swap_size} /swapfile && chmod 600 /swapfile && mkswap /swapfile",
            ],
            phase="swapfile",
        )
    ...
    run_in_chroot(
        state,
        ["bash", "-lc", "systemctl mask cups 2>/dev/null || true"],
        phase="mask-cups",
    )
    run_in_chroot(
        state,
...
def write_fstab(state: State) -> None:
    ...
    if config.home_partition:
        blkid_vars.append(
            f"home_uuid=$(/sbin/blkid -s UUID -o value {config.home_partition})"
        )
    if config.swap_partition:
        blkid_vars.append(
            f"swap_uuid=$(/sbin/blkid -s UUID -o value {config.swap_partition})"
        )
    ...
    fstab_content = "\n".join(fstab_lines)
    shell = (
        " && ".join(blkid_vars) + f" && cat > /etc/fstab <<EOF\n{fstab_content}\nEOF"
    )
    run_command(
        ["chroot", config.target_mount, "bash", "-lc", shell],
        phase="fstab",
        state=state,
    )
...
def create_users(state: State) -> None:
    ...
    run_in_chroot(
        state,
        ["adduser", "--disabled-password", "--gecos", "", config.username],
        phase="user",
    )
...
def install_bootloader(state: State) -> None:
    run_in_chroot(
        state, ["apt", "install", "-y", "grub-efi-amd64"], phase="install-grub"
    )
    run_in_chroot(
        state,
        [
...
def prompt_backports_kernel(state: State) -> None:
    ...
    run_in_chroot(
        ...
        [
            ...
            "-y",
            "-t",
            f"{state.config.release}-backports",
            "linux-image-amd64",
        ],
        phase="backports-kernel",
    )
